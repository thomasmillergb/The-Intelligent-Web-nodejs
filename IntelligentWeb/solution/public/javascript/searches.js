...
function getFormData(element) {
    ...
    element.find('[name]').each(function() {
        if ($(this).is('[type=checkbox]')) formData[this.name] = $(this).is(':checked');
...
$(function() {
    ...
    $("[name='days']").on("keyup", function() {
        if (parseInt($(this).val()) <= 0 || parseInt($(this).val()) == NaN) $(this).parent().parent().parent().parent().parent().find("[name='liveresults']").prop('checked', true);
        else $(this).parent().parent().parent().parent().parent().find("[name='liveresults']").prop('checked', false);
    });
    ...
    $("[name='liveresults']").click(function() {
        if ($(this).is(':checked')) {
            ...
            if (parseInt($(this).parent().parent().parent().parent().parent().parent().find("[name='days']").val()) <= 0) $(this).parent().parent().parent().parent().parent().parent().find("[name='days']").val('5');
        }
    });
    // Discussion search
            ...
            socket.emit('discussion_search', getFormData($(this)), function(err, data) {
                ...
                if (err != null) {
                    ...
                    if ($("#discussion_search_form input[name=liveresults]").is(':checked')) {
                        ...
                        for (i = 0; i < data.tweets.length; i++) appendTweetWithAccount($("#discussion_tweet_return"), data.tweets[i], true);
            ...
            socket.emit('user_venues_search', getFormData($(this)), function(err, data) {
                ...
                if (err != null) {
                    ...
                    if ($("#user_venues_search_form input[name=liveresults]").is(':checked')) {
                        ...
                        //console.log(data);
                        $("#user_venues_return").html("<hr>" + '<table style="padding:0;background:none;"><tr><td>' + "<h1>User venues</h1>Here are the venues for" + '<span xmlns:assignment="http://127.0.0.1/rdf/rdf.xml">' + '<span about="http://127.0.0.1:3000/twitteruser/data.rdf#' + data.user.screen_name + '" typeof="assigment:twitterUser">' + '<a href="http://www.twitter.com/' + data.user.screen_name + '">@' + data.user.screen_name + '</a>' + '<span style="display:none;" property="assigment:screenName">' + data.user.screen_name + '</span>' + '<span style="display:none;" property="assigment:id">' + data.user.id_str + '</span>' + '<span style="display:none;" property="assigment:location">' + data.user.location + '</span>' + '<span style="display:none;" property="assigment:profilePicture">' + data.user.profile_image_url + '</span>' + '<span style="display:none;" property="assigment:realName">' + data.user.name + '</span>' + '</span>' + '</span>' + "<br><a href=\"javascript:void(0)\" onclick=\"getUserAndTweets('" + data.user.screen_name + "')\">View user's profile and Tweets</a>" + '</td><td>' + "<br><div><span class=\"extra\">Extra</span> <a style=\"display: inline-block;\" href=\"persistentqueries/" + data.queryid + "\" target=\"_blank\" class=\"button\">Persitent file with RDFa</a></div>" + '</td></table>');
                        visitedVenues($("#user_venues_return"), data.visitedvenuestable);
...
function getPointsOfInterest(name, lat, long) {
    ...
    socket.emit('get_points_of_interest', name, lat, long, function(err, data) {
        ...
        if (err != null) {
            ...
            for (var indx in data.venues) data.venues[indx].label = "<h3>" + data.venues[indx].label + "</h3><a href='" + data.venues[indx].link + "' target='_blank'>" + data.venues[indx].link + "</a>";

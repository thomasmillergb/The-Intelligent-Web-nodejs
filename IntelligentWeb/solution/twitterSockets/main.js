/**
THe main class for sockets, contain all socketing lising and carries out all twitter Search:
	Discussion
	Users discussion
	User venues
	Venue
...
io.on('connection', function(socket) {
    ...
    var four_token;
    /**
	Stores local twitter access tokem
	param temp_twitter_token   cleints twitter token
	paran temp_twitter_token_secret  cleints twitter token secret
	param fn return data or error message
	*/
    ...
    /**
	Stores local foursqaure access tokem
	param temp_twitter_token   cleints twitter token
	paran temp_twitter_token_secret  cleints twitter token secret
	param fn return data or error message
	*/
	...
	*/
    socket.on('discussion_search', function(params, fn) {
        ...
        //Streaming API    
        ////was not used in final version, due limitation of twitter streaming api
        if (params.liveresults) {
            console.log(filterParams);
            currentTwitStream = twitterAPI.stream('statuses/filter', filterParams, function(stream) {
                //console.log(bounds);
                stream.on('data', function(data) {
                    if (!params.uselocation || (params.uselocation && data.coordinates && data.coordinates.coordinates[1] >= bounds[0] && data.coordinates.coordinates[
                        1] <= bounds[2] && data.coordinates.coordinates[0] >= bounds[1] && data.coordinates.coordinates[0] <= bounds[3])) {
                        var tempData = {};
                        tempData.tweet = data;
                        tempData.marker = [];
    ...
    /**
	Performs user disussion search
		// params.screennames
		// params.keywords
		// params.days
		// params.liveresults
	...
	*/
    ...
    /**
	Performs user disussion search
	params keywordsTable   the current state of keyword table
	params userid          the twitter user id
	params words           the words to be added to the keyword table
	*/
                            ...
                            foursqaure.getVenues(data, four_token, function(checkIns) {
                                if (checkIns != null) {
                                    ...
                                    checkIns.forEach(function(checkinAndID, idx) {
                                        ...
                                        returndata.markers.label = "<h3>@" + checkin.user.firstName + " " + checkin.user.lastName + "</h3>" +
                                            checkin.shout + "";
                ...
                twitterRestAPI.get('statuses/user_timeline', searchParams, function(err, data, response) {
                    if (!err) {
                        ...
                        if (params.twitterfoursquare == 'foursquare') {
                            if (data != "") {
                                var dataoutput = {};
                                if (data.length > 0) dataoutput.user = data[0].user;
                                foursqaure.getVenues(data, four_token, function(checkIns) {
	...
	*/
	...
	Gets the users tweets
		params screenname  the twitter user screen name
		params fn returns the data
	*/
    ...
    /**
	Gets the users tweets
		params name  name of point of intrest
		params lat lat of location
		params long  long of location
		params fn returns the data
	*/
    socket.on('get_points_of_interest', function(name, lat, long, fn) {
        ...
        var query = 'PREFIX geo: <http://www.w3.org/2003/01/geo/wgs84_pos#>			     ' + 'SELECT ?subject ?link ?image_link ?comment ?label ?lat ?long WHERE {					' +
            '															     ' + '	?subject geo:lat ?lat.									     ' + '	?subject geo:long ?long.								     ' + '	?subject rdfs:label ?label.								     ' +
            '	?subject rdfs:label ?distance.							     ' + '	?subject foaf:isPrimaryTopicOf ?link.						' + '	?subject foaf:depiction ?image_link.								' +
            '	?subject rdfs:comment ?comment.						     ' + '															     ' + '	FILTER(													     ' + '		xsd:double(?lat) - xsd:double(\'' + lat + '\') <= 0.05 &&	     ' +
            '		xsd:double(\'' + lat + '\') - xsd:double(?lat) <= 0.05 &&	     ' + '		xsd:double(?long) - xsd:double(\'' + long + '\') <= 0.05 &&    ' + '		xsd:double(\'' + long +
            '\') - xsd:double(?long) <= 0.05 &&    ' + '		lang(?label) = "en"									     ' + '	).														     ' + '}															     ' +
            'ORDER BY ASC((xsd:double(?lat) - xsd:double(\'' + lat + '\')) * (xsd:double(?lat) - xsd:double(\'' + lat + '\')) + (xsd:double(\'' + long +
            '\') - xsd:double(?long)) * (xsd:double(\'' + long + '\') - xsd:double(?long)))						 ' + 'LIMIT 10';
        ...
        foursqaure.getVenuesFromLocation(lat + ',' + long, four_token, function(err, venuedata) {
            if (err == null) {
                ...
                for (var index in venuedata.response.venues) {
                    var name = "";
                    if (venuedata.response.venues[index].categories[0]) name = venuedata.response.venues[index].categories[0].name;
                    var link = "https://foursquare.com/v/" + venuedata.response.venues[index].id;
                    if (venuedata.response.venues[index].url) link = venuedata.response.venues[index].url;
                    var address = venuedata.response.venues[index].location.address + ', ' + venuedata.response.venues[index].location.postalCode + ', ' + venuedata.response.venues[
                        index].location.country
                    returnData.venues.push({
                        'link': link,
                        'label': venuedata.response.venues[index].name,
                        'lat': venuedata.response.venues[index].location.lat.toFixed(6),
                        'long': venuedata.response.venues[index].location.lng.toFixed(6),
                        'image_link': 'none',
                ...
                returnedsources++;
                if (returnedsources == 2) fn(null, returnData);
            } else {
                fn(err, null);
            }
        });
        client.query(query).execute(function(error, results) {
            if (!error) {
                for (var i = 0; i < results.results.bindings.length; i++) {
                    ...
                    returnData.venues.push({
                        'link': currentrow.link.value,
                        'label': currentrow.label.value,
                        'lat': currentrow.lat.value,
                        'long': currentrow.long.value,
                        'image_link': currentrow.image_link.value,
                        'category': '',
                        'description': currentrow.comment.value.substring(0, 100),
                        'from': 'DBPedia'
                    });
                }
                returnedsources++;
    ...
    /**
	Gets the users tweets
		params user_id  the twitter user id
		params fn returns the data
	*/
	...
	Gets alls users who have been to the venue
		params venue_id  the venue id of the venue
		params name   name of the user
		params foursquare  if foursqaure or twitter request
		params fn returns the data
	*/
    ...
    /**
	returns a list of users who meat the search params
		params params  the search params
		params fn returns the data
	*/
...
/**
	sorts the results from the venues DB and sorts it into order
		params venues  the rows of venues from the MySQL search
		params foursquare if foursqaure or twitter
	*/
function dbVenueSorting(venues, foursquare) {
        ...
        for (var i = 0; i < venues.length; i++) {
            ...
            if (!found) {
                ...
                if (venues[i].venue_id == null) temp.venue_id = venues[i].venue_id;
                else temp.venue_id = venues[i].venue_id;
    ...
    /**
	sorts the results from the users DB and sorts it into order
		params users  the rows of users from the MySQL search
		params foursquare if foursqaure or twitter
	*/
...
function dbUserSorting(users, foursquare) {
    ...
    /**
Gets the markers out of tweets and return as marker for google maps
	params data  tweets
*/
...
function getMarkers(data) {
    ...
    /**
Gets the userid and screen names out of search params
	params users  usernames or user id
*/
...
function getUserIDAndScreenName(user, callback) {
    ...
    /**
Gets the userid  out of search params 
...
function asyncGetUserScreenName(id, callback) {
    ...
    /**
Works out the bound box
	params centerPoint the centre of the area
	params distance  the distance from the centre
*/
...
function getBoundingBox(centerPoint, distance) {
    ...
    /**
Reformats the twitter data
	params aDate  the date to reformat
*/
...
function parseTwitterDate(aDate) {
    ...
    /**
Removes all tweets which are not with the days param
	params data the tweets
	params dats the amount of days to search back from
*/
...
var exampleUserJson = {
    ...
    "id_str": "308358479",
    "name": "James McIlveen",
    "screen_name": "jtmcilveen",
    "location": "Whaley Bridge",
    "profile_location": null,
    "description": "Rock climber (bouldering) and computer scientist doing my Masters at Sheffield University",
    "url": "http://t.co/BiYvW71Wux",
    "entities": {
        "url": {
            "urls": [{
                "url": "http://t.co/BiYvW71Wux",
                "expanded_url": "http://www.jamesmcilveen.com",
                "display_url": "jamesmcilveen.com",
                "indices": [0, 22]
            }]
        },
        "description": {
    ...
    "friends_count": 31,
    "listed_count": 0,
    "created_at": "Tue May 31 08:22:02 +0000 2011",
    "favourites_count": 9,
    "utc_offset": 0,
    "time_zone": "London",
    "geo_enabled": false,
    ...
    "profile_image_url": "http://pbs.twimg.com/profile_images/490139247945719809/GHt2iObY_normal.jpeg",
    "profile_image_url_https": "https://pbs.twimg.com/profile_images/490139247945719809/GHt2iObY_normal.jpeg",
    "profile_banner_url": "https://pbs.twimg.com/profile_banners/308358479/1405693222",
...
var exampleTweetJson_1 = {
    "coordinates": [53.381796, -1.480719],
    "favorited": false,
    "truncated": false,
    ...
    "contributors": null,
    "text": "This is an #example tweet 1111111111",
    "metadata": {
    ...
    "place": null,
    "user": exampleUserJson,
    "in_reply_to_screen_name": null,
    "source": "<a>Twitter for Mac</a>",
    "in_reply_to_status_id": null
};
var exampleTweetJson_2 = {
    "coordinates": [53.3816232, -1.4817597],
    "favorited": false,
    "truncated": false,
    "created_at": "Mon Sep 24 03:35:21 +0000 2012",
    "id_str": "250075927172759552",
    ...
    "in_reply_to_user_id_str": null,
    "contributors": null,
    "text": "This is an #example tweet 2222222222",
    "metadata": {
        "iso_language_code": "en",
        "result_type": "recent"
    },
...
var exampleMarkerJson = [{
    "label": "<h3>@jtmcilveen</h3>Random tweet number 1",
    "lat": 53.381796,
    "long": -1.480719
}, {
    "label": "<h3>@jtmcilveen</h3>Random tweet number 2",
    "lat": 53.3816232,
    "long": -1.4817597
}];
var databaseUserJson = [{
    "user_id": "839249234",
    "user": "jtmcilveen",
    "name": "James McIlveen"
}, {
...
/**
creates a venutes visted table about of the data
	params data the tweets
	params foursqaure if to draw a foursqaure table or a twitter table
*/
function visitedVenuesHTML(data, foursquare) {
        ...
        tablehtml += "<h1>User venues</h1>Here are the venues for" + '<span xmlns:assignment="http://127.0.0.1/rdf/rdf.xml">' + '<span about="http://127.0.0.1:3000/twitteruser/data.rdf#' + data.user.screen_name +
            '" typeof="assigment:twitterUser">' + '<a href="http://www.twitter.com/' + data.user.screen_name + '">@' + data.user.screen_name + '</a>' +
            '<span style="display:none;" property="assigment:screenName">' + data.user.screen_name + '</span>' + '<span style="display:none;" property="assigment:id">' + data.user.id_str + '</span>' +
            '<span style="display:none;" property="assigment:location">' + data.user.location + '</span>' + '<span style="display:none;" property="assigment:profilePicture">' + data.user.profile_image_url +
            '</span>' + '<span style="display:none;" property="assigment:realName">' + data.user.name + '</span>' + '</span>' + '</span>' +
            "<br><a href=\"javascript:void(0)\" onclick=\"getUserAndTweets('" + data.user.screen_name + "')\">View user's profile and Tweets</a>";
        if (foursquare) {
            tablehtml +=
                '<table class="tweet_results_table" cellspacing="0" xmlns:assignment="http://127.0.0.1/rdf/rdf.xml"><tr><td>Picture</td><td>Venue</td><td>Description</td><td>Rating</td><td>Likes</td><td>Lat/Long</td><td>Address</td><td>Number of visits</td><td>Latest visit date</td></tr>';
            for (i = 0; i < data.visitedvenuestable.length; i++) {
                ...
                if (row.bestPhoto == 'undefined' || row.bestPhoto == 'none') image = "No photo";
                else image = '<img property="assigment:imageURI" src="' + row.bestPhoto + '" class="tableimage" />';
            ...
            tablehtml +=
                '<table class="tweet_results_table" cellspacing="0" xmlns:assignment="http://127.0.0.1/rdf/rdf.xml"><tr><td>Venue</td><td>Lat/Long</td><td>Number of visits</td><td>Last visited</td><td>Points of interest</td></tr>';
            for (i = 0; i < data.visitedvenuestable.length; i++) {
                row = data.visitedvenuestable[i];
                tablehtml += '<tr about="http://127.0.0.1:3000/venues/data.rdf#' + row['venue'].replace(/\W/g, '') + '" typeof="assigment:venue">';
                ...
                tablehtml += '<td><a href="javascript:getPointsOfInterest(\'' + row['venue'].replace("'", "") + '\', \'' + row['lat'] + '\', \'' + row['long'] +
                    '\')">See points of interest close by</a></td>';
                tablehtml += '</tr>';
            }
            tablehtml += '</table>';
        }
    ...
    /**
creates a rdfa out of the venue visted 
	params dataoutput is the data to be saved in the rdfa
	params foursqaure if to draw a foursqaure table or a twitter table
*/
